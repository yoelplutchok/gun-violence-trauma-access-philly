...
**Background:** The "golden hour" principle in trauma care establishes that survival rates for penetrating injuries decline significantly when definitive surgical care is delayed. This study examines whether Philadelphia neighborhoods with the highest gun violence burden face systematic barriers to timely Level I trauma care, creating "trauma deserts" that compound existing health disparities.

**Methods:** We analyzed 17,380 shooting incidents in Philadelphia (2015-2025) using bivariate classification of shooting density and modeled transport time to the nearest Level I Adult trauma center. Drive-time isochrones were generated via the OpenRouteService API. We identified trauma desert tracts (high violence burden AND poor access), characterized their demographics, and tested the association between transport time and fatality using logistic regression.

**Results:** We identified 18 census tracts (4.4% of the city) as trauma deserts, containing 83,159 residents who experienced 1,659 shootings (9.5% of total). Paradoxically, 99.6% of all shootings occurred within 20 minutes of Level I trauma care. Predominantly Black tracts were actually 3.2 minutes closer to trauma centers on average, yet experienced 4.4-fold higher shooting density. Oaxaca-Blinder decomposition revealed that 68.5% of this disparity is unexplained by observable socioeconomic factors. Critically, fatality rates increased from 20.0% in areas 0-5 minutes from trauma care to 30.6% in areas 20+ minutes away (OR=1.015 per minute, 95% CI: 1.005-1.025, p=0.005).

**Conclusions:** The "trauma desert" phenomenon in Philadelphia is driven primarily by extreme concentration of gun violence rather than geographic access barriers. While transport time significantly affects mortality, the disparity in trauma burden—not distance to care—represents the primary equity concern. Interventions should prioritize violence prevention in affected neighborhoods alongside community-based hemorrhage control training.
...
This study addresses a fundamental question: **Is there a spatial mismatch between gun violence burden and timely access to Level I trauma care in Philadelphia, and does this mismatch disproportionately affect predominantly Black and low-income communities?**

We operationalize this through the concept of "trauma deserts"—census tracts that simultaneously experience high shooting density AND extended transport times to Level I trauma centers.
...
We conducted a retrospective geospatial analysis of shooting incidents in Philadelphia, Pennsylvania from January 1, 2015 through December 31, 2025. Philadelphia is a densely populated urban center (population 1.59 million) served by four Level I Adult trauma centers, two Level I Pediatric centers, and one Level II Adult trauma center.

### Data Sources

#### Shooting Incident Data
Shooting victim data was obtained from OpenDataPhilly, the City of Philadelphia's open data portal. The dataset includes all reported shooting incidents with geocoded coordinates, victim demographics (age, sex, race), wound location, and fatality status. After removing 27 records with invalid coordinates, the analysis dataset includes 17,383 shooting incidents.
...
Travel time polygons were generated using the OpenRouteService API with a driving-car profile. For each Level I Adult trauma center, isochrones were computed at 5, 10, 15, 20, and 30 minute intervals representing the geographic area reachable within each time threshold.
...
Shooting density was calculated as total shootings per square mile per year for each tract, standardizing for tract size and study duration (11 years).

### Bivariate Classification

We employed a 3×3 bivariate classification matrix combining shooting density terciles and transport time terciles (Table 1). "Trauma deserts" were defined as Class 9 tracts—those in the highest tercile for both shooting density AND transport time.
...
Demographic comparisons between trauma desert and non-trauma desert tracts were conducted using Welch's t-test. The association between transport time and fatality was tested using chi-square test for categorical analysis and logistic regression for continuous estimation of odds ratios.

Oaxaca-Blinder decomposition was applied to quantify racial disparities, separating tracts into predominantly Black (≥50% Black population, n=143) and other tracts (n=246). Shooting density was log-transformed for the decomposition. All analyses used α=0.05 for significance.

### Ethical Considerations

This study used publicly available, de-identified administrative data and did not require IRB approval.
...
The bivariate classification identified **18 census tracts (4.4% of the city)** as trauma deserts (Table 2). These tracts contain 83,159 residents (5.2% of the city population) and experienced 1,659 shootings over the study period (9.5% of total shootings).
...
Over 90% of shootings occur within 10 minutes of Level I trauma care, indicating that geographic access is not the primary barrier to timely treatment.
...
Fatality rate increased monotonically from 20.0% in areas closest to trauma care to 30.6% in areas 20+ minutes away—an absolute difference of 10.7 percentage points. This association was statistically significant:

- **Chi-square test:** χ² = 11.46, df = 4, p = 0.022
- **Logistic regression:** OR = 1.015 per minute (95% CI: 1.005-1.025, p = 0.005)

Each additional minute of transport time was associated with 1.5% higher odds of fatality.
...
Trauma deserts are disproportionately located in predominantly Black and high-poverty neighborhoods (Table 5).
...
However, a counterintuitive finding emerges when examining the citywide relationship: **predominantly Black neighborhoods are actually closer to Level I trauma centers on average.** The correlation between percent Black population and transport time is negative (r = -0.25, p < 0.0001).
...
Predominantly Black tracts experience **4.4-fold higher shooting density** (geometric mean ratio). Only 31.5% of this disparity is explained by differences in poverty rates and median household income. **68.5% of the gap is unexplained**—attributable to structural and historical factors not captured by current socioeconomic indicators.
...
![Figure 3: Oaxaca decomposition](../figures/oaxaca_decomposition.png)
...
Temple University Hospital's catchment area contains over half of all shootings in the city. Combined, Temple and Penn Presbyterian serve catchment areas containing 81.8% of shootings.
...
Peak year was 2021 with 2,338 shootings—nearly double the pre-pandemic average. Violence has since declined but remains elevated above pre-2020 levels.
...
We constructed a Neighborhood Vulnerability Index (NVI) combining poverty rate, household income, shooting density, and transport time. Trauma deserts showed elevated vulnerability: average NVI of 21.6 versus 16.5 citywide (1.31× higher). Nine of 18 trauma deserts (50%) fell in the top quartile of vulnerability.

![Figure 6: Vulnerability map](../figures/vulnerability_index_map.png)
...
This study reveals a paradox at the heart of Philadelphia's trauma desert geography: while 18 census tracts meet our bivariate criteria for trauma desert status, **the primary driver of elevated trauma burden is violence concentration, not geographic access barriers.**
...
Our finding that fatality rates increase from 20.0% to 30.6% across the transport time gradient confirms that transport time remains clinically relevant. This aligns with published literature on trauma transport and mortality (Newgard et al., 2010; Byrne et al., 2019). However, because such a small fraction of shootings occur in high-transport-time areas, improving geographic access alone would have limited population-level impact.

### The Unexplained Disparity

Perhaps the most significant finding is that 68.5% of the shooting density disparity between Black and other tracts cannot be explained by current poverty or income differences. This unexplained portion likely reflects:
...
Our findings suggest that traditional approaches to improving trauma access—building new facilities or redistributing EMS resources—would have limited impact in Philadelphia given the already-excellent geographic coverage. Instead, interventions should focus on:
...
The "trauma desert" phenomenon in Philadelphia is driven primarily by the extreme concentration of gun violence in specific neighborhoods rather than by poor geographic access to Level I trauma care. While transport time significantly affects mortality outcomes, the overwhelming majority of shootings occur within excellent proximity to trauma centers.

The 4.4-fold disparity in shooting density between predominantly Black and other tracts—68.5% of which is unexplained by observable socioeconomic factors—represents a fundamental health equity crisis that cannot be addressed through trauma system redesign alone. Reducing gun violence burden in affected neighborhoods must be the primary policy objective.
